...
ifeq ($(config),debug_x64)
  ...
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -no-pie
...
ifeq ($(config),release_x64)
  ...
  TARGETDIR = bin
  TARGET = $(TARGETDIR)/loopback
  OBJDIR = obj/x64/Release/loopback
  DEFINES += -DNDEBUG
  INCLUDES += -I. -I/usr/local/include -Inetcode.io -Ireliable.io
  FORCE_INCLUDE +=
  ALL_CPPFLAGS += $(CPPFLAGS) -MMD -MP $(DEFINES) $(INCLUDES)
  ...
  LIBS += bin/libyojimbo.a -lsodium -lmbedtls -lmbedx509 -lmbedcrypto
  LDDEPS += bin/libyojimbo.a
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -s -no-pie
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
...
$(OBJDIR)/loopback.o: loopback.cpp
	@echo $(notdir $<)
ifeq (posix,$(SHELLTYPE))
	$(SILENT) mkdir -p $(OBJDIR)
else
